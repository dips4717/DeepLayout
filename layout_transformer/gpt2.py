...
class CondBlock(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        self.ln_2 = nn.LayerNorm(nx)
        self.mlp = MLP(4 * nx, config)
        self.mlp_cond = MLP(4 * nx, config)

    def forward(
        self, x, cond, layer_past=None, attention_mask=None, head_mask=None, use_cache=False, output_attentions=False,
    ):
        output_attn = self.attn(
            self.ln_1(x),
            layer_past=layer_past,
        ...
        m = self.mlp(self.ln_2(x))
        x = x + m
        m_cond = self.mlp_cond(self.ln_2(cond.unsqueeze(dim=1)))
        x = x + m_cond
...
class CondBlock2(nn.Module):
    """" Same as CondBlock but returns only one tensor not list"""
    def __init__(self, n_ctx, config, scale=False):
        super().__init__()
        nx = config.n_embd
        self.ln_1 = nn.LayerNorm(nx)
